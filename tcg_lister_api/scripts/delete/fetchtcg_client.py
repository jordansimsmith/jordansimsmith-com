...
MANAGED_LISTING_DELETE_PATH_PATTERN = re.compile(r"^/v1/manage-listings/[1-9][0-9]*$")
CONDITION_QUALITY = {
    "raw-d": 0,
    "raw-hp": 1,
    "raw-mp": 2,
...
class FetchTcgClient:
    ...
    def delete_managed_listing(self, listing_id):
        listing_id = self._required_positive_int(listing_id, "listing id")
        self._request(
            f"{MANAGED_LISTINGS_PATH}/{listing_id}",
            method="DELETE",
            authorization=self._authorization_header(),
        )
    ...
    def _parse_managed_listing(self, value):
        ...
        name = self._required_string(card.get("name"), "managed listing card name")
        collector_number = self._required_string(
            card.get("cardCode"), "managed listing collector number"
        )
        ...
        set_name = self._required_string(
            fetch_set.get("displayName"), "managed listing set displayName"
        )
    ...
    def _request_json(
        ...
        *,
        ...
        if not isinstance(payload, dict):
            raise FetchTcgRequestError(f"Fetch request failed for {path}")
        return payload
    ...
    def _request(
        ...
        *,
        ...
        authenticated = self._is_authenticated_request(method, path)
        if authorization is not None and not authenticated:
            raise RunSafetyStop("authenticated request path was not explicitly allowed")
        if authenticated and authorization is None:
            raise RunSafetyStop("authenticated request was missing authorization")
        if method != "GET" and not self._is_allowed_delete(method, path):
        ...
        for attempt in range(1, MAX_ATTEMPTS + 1):
            ...
            if self._is_allowed_delete(method, path):
                if not self._response_body_is_empty(response):
                    raise FetchTcgRequestError("listing delete response was not empty")
                return None
    ...
    @staticmethod
    def _is_authenticated_request(method, path):
        if method == "GET" and path == MANAGED_LISTINGS_PATH:
            return True
        return FetchTcgClient._is_allowed_delete(method, path)

    @staticmethod
    def _is_allowed_delete(method, path):
        return method == "DELETE" and bool(
            MANAGED_LISTING_DELETE_PATH_PATTERN.fullmatch(path)
        )

    @staticmethod
    def _response_body_is_empty(response):
