...
- Scan review selects a printing catalog by game and consumes generic printing identity and display fields. The Magic catalog adapts browser-direct Scryfall search, paginated `prints_search_uri` results, card metadata, and reference images; it handles double-faced cards through `card_faces[].image_uris` when top-level `image_uris` is absent. Confirmation stores the selected printing's own `external_source` and `external_id`. No browser score is presented as calibrated confidence.
...
- **Scan printing catalogs**: scan review selects a catalog adapter by game and uses its generic printing identity, search, and display contract. The registered Magic adapter calls Scryfall directly for name search, all pages of a card's `prints_search_uri`, chosen-printing metadata, and reference images. It uses `card_faces[].image_uris` for double-faced cards without top-level `image_uris`. The API does not proxy these calls; only the final selected fields are sent to Confirm scan.
...
- Scan review requires an explicit printing confirmation for every retained row, even a strong suggestion. The browser stores the selected catalog printing's `external_source`, `external_id`, name, set code/name, and collector number; changing the selected printing clears confirmation. The Magic catalog enforces `lang=en` and the batch finish (`normal` maps to Scryfall `nonfoil`), and keeps alternate art, double-faced cards, and tokens distinct. A `needs_review` row without a useful suggestion can be resolved through manual search.
