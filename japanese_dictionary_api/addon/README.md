...
- Build the prospective note in memory: `Word` (kana form picks `reading`, otherwise `expression`), `Reading`, `Glossary` (Python port of Yomitan's `structured-content-generator.js` with bundled Yomitan `structured-content-style.json` and Jitendex `styles.css` rules inlined as `style` attributes on each matching element, classes stripped — matching Yomitan's `CssStyleApplier.applyClassStyles` + `_normalizeHtml`), `Graph` (Python port of Yomitan's `pronunciation-generator.js` simple-style SVG, inline styles), `Audio` (jpod101 URL → MP3 download → `[sound:<filename>]`).
...
- Shipping a separate CSS file, a `<style>` block, or modifying the user's `Animecards` card template / stylesheet. Both the Yomitan structured-content rules and the Jitendex visual rules are inlined as `style` attributes on each element. This is required because the Animecards card template's cycling script replaces `#glossary-field` innerHTML with individual `<li>` contents, destroying any `<style>` block but preserving inline styles.
...
- **Faithful Yomitan port for glossary HTML and pitch graph SVG**: matches the visual style of cards the user already makes with Yomitan + AnkiConnect. `render_field` builds the structured-content tree, then `css_inline.inline_styles` walks every element and applies matching rules from the bundled Yomitan `structured-content-style.json` (`.gloss-link-external-icon` hider, `.gloss-sc-*` table chrome) and Jitendex `styles.css` as per-element `style` attributes, then strips `class` attributes — matching Yomitan's `CssStyleApplier.applyClassStyles` + `_normalizeHtml`. The Yomitan rules live in `addon/css_inline.py` (relevant subset — the `.gloss-image-*` family is intentionally omitted because we render image nodes as `[image: …]` placeholders) and the Jitendex rules in `addon/jitendex_styles.py` (revision 2026.05.05.0). `addon/css_scope.py` flattens the Jitendex `&` nesting and scopes selectors to `.structured-content` before the inliner matches them. Update `jitendex_styles.py` by replacing the constant when the upstream dictionary cuts a new revision. Styles are inlined rather than shipped in a `<style>` block because the Animecards card template's cycling script replaces `#glossary-field` innerHTML with individual `<li>` contents, destroying any `<style>` block but preserving inline styles on elements within each `<li>`. Jitendex moved its visual rules out of the per-element JSON `style` field in version 2025.08.01.0, so per-element style propagation through `_apply_style` is no longer sufficient on its own — the CSS inliner is the load-bearing path. The pitch-graph SVG carries an inline `style="display:inline-block;vertical-align:middle;height:1.5em;"` matching Yomitan's `display-pronunciation.css` so the graph sits at one line of text height instead of stretching to its intrinsic 100px viewBox.
...
| Glossary stylesheets    | Yomitan `structured-content-style.json` (GPL-3.0) + Jitendex `styles.css` (CC BY-SA 4.0) | Yomitan subset in `addon/css_inline.py`, Jitendex rules in `addon/jitendex_styles.py` (revision 2026.05.05.0); replace each constant on its respective upstream bump |
