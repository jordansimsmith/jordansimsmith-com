...
- Shipping a separate CSS file or modifying the user's `Animecards` card template / stylesheet. Both the Yomitan structured-content rules and the Jitendex visual rules ride along inside each glossary field as a scoped `<style>` block (~7 KB per card); the pitch-graph SVG keeps inline styles only.
...
sequenceDiagram
  ...
  A-->>U: non-modal confirmation dialog (imported / dropped / failed) -> clear server bookmarks?
  alt user confirms
...
- **Non-modal confirm-before-delete checkpoint**: between the import and cleanup `QueryOp`s the addon shows a non-modal `QMessageBox` summarising the import. The user can browse their deck to inspect the imported cards while the dialog stays open, then click `Clear bookmarks from server` (run cleanup) or `Keep bookmarks queued` (skip cleanup). Notes are already in Anki regardless of the answer; `Keep` only suppresses the server-side `DELETE /bookmarks/{sequence}` calls. Rationale: a single dropped row or wrong deck pick used to be silently destructive because the bookmark was wiped before the user could see the imported note; the non-modal checkpoint lets the user verify first.
...
- **Faithful Yomitan port for glossary HTML and pitch graph SVG**: matches the visual style of cards the user already makes with Yomitan + AnkiConnect. The glossary field shape mirrors `tmp/yomitan/ext/data/templates/default-anki-field-templates.handlebars` for `definition.type === "term"`: the structured-content tree (with `gloss-sc-*` classes and `data-sc-*` attributes intact) is wrapped in `<div class="yomitan-glossary" style="text-align: left;">…</div>` and followed by a `<style>` block carrying two stylesheets concatenated and wrapped in `.yomitan-glossary { … }` (CSS nesting), equivalent to `addScopeToCss(styles, '.yomitan-glossary')` from `tmp/yomitan/ext/js/core/utilities.js`. Stock Yomitan inlines its own `structured-content-style.json` per element via `CssStyleApplier.applyClassStyles` and ships the dictionary's `styles.css` as a `<style>` block; we collapse both into the same `<style>` block so a single seam owns all Anki-side visual rendering. The bundles live verbatim in `addon/yomitan_styles.py` (relevant subset of Yomitan's `structured-content-style.json` covering `.gloss-link-external-icon` and `.gloss-sc-*` table chrome — the `.gloss-image-*` family is intentionally omitted because we render image nodes as `[image: …]` placeholders) and `addon/jitendex_styles.py` (Jitendex revision 2026.05.05.0). Update by replacing each constant when its upstream cuts a new revision. Yomitan rules are emitted before Jitendex rules so the more-specific Jitendex attribute selectors win on overlap. Jitendex moved its visual rules out of the per-element JSON `style` field in version 2025.08.01.0, so per-element style propagation through `_apply_style` is no longer sufficient on its own — the bundled stylesheets are the load-bearing path. The pitch-graph SVG carries an inline `style="display:inline-block;vertical-align:middle;height:1.5em;"` matching Yomitan's `display-pronunciation.css` so the graph sits at one line of text height instead of stretching to its intrinsic 100px viewBox.
...
| Glossary stylesheets    | Yomitan `structured-content-style.json` (GPL-3.0) + Jitendex `styles.css` (CC BY-SA 4.0) | bundled verbatim into `addon/yomitan_styles.py` (relevant `.gloss-sc-*` / `.gloss-link-external-icon` subset) and `addon/jitendex_styles.py` (Jitendex revision 2026.05.05.0); replace each constant on its respective upstream bump |
