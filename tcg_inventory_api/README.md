...
- Scan tests cover ASCII filename ordering, batch condition/finish validation, rejection of non-ASCII names, complete-batch upload verification with no replacement URLs, owner-scoped source URLs, single-attempt worker results, duplicate SQS delivery, DFC/token/alternate-art fixtures, irreversible scan-row and whole-scan deletion, deletion fences for stale identify/worker writes, direct import parent/row/job writes, confirmed retry response, and first-scanned-first-import row order. The worker's packaged catalog/model must open offline with network disabled.
