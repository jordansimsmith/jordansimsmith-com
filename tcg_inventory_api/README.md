...
- Appraisal selects a catalog resolver by the import's registered game. The Magic resolver owns its English-language rule, Scryfall set mapping, front-face name normalization, FetchTCG game and finish values, and the `externalReferences.scryfallId` field. The shared FetchTCG card resolver searches each supplied set, fetches candidate details, verifies the external reference, and extracts NZ market price. Shared appraisal owns duplicate-resolution caching, rival pricing, and keep/discard policy. Search ranking is never trusted, so multiple printings of one name in a set (borderless, showcase, extended art) cannot collapse onto the wrong card; a row with no verified candidate becomes `review`.
...
- **SKU**: game + external source + external printing ID + finish + condition; the sellable identity. One FetchTCG listing per SKU. Permanent once created.
...
- **Scryfall API**: the set-mapping generator consumes public set/card records. In Magic scanner intake, the browser calls Scryfall directly for search, paginated printings, card metadata, and reference images, including face images for double-faced cards. Order detail returns each target unit's `game`, `external_source`, and `external_id`; the browser's image resolver currently maps only Magic/Scryfall identity to a small card image, without an API-side lookup. Scan confirmation receives the browser-selected `external_source`, `external_id`, and row metadata; it makes no Scryfall lookup. Existing FetchTCG appraisal verifies the Scryfall ID against its candidate before a keep decision.
...
- `sku_id` is `<game>#<external_source>#<external_id>#<finish>#<condition>`. A SKU record and its unit items share a partition so one query serves detail, recount, and allocation.

| Item             | pk                            | sk                                              | Notable attributes                                                                                                                                                                                                                                                                                                                            |
| ---------------- | ----------------------------- | ----------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| SKU              | `USER#<u>#SKU#<sku_id>`       | `SKU`                                           | game, external_source, external_id, finish, condition, name, set_code, set_name, collector_number, fetchtcg_card_id, fetchtcg_set_id, `version`, `dirty`, `fetchtcg_listing_id`, `last_published_quantity`, `last_published_price`, `last_published_at`                                                                                       |
...
| Import           | `USER#<u>`                    | `IMPORT#<ulid>`                                 | game, filename, status, row counts, error (when the appraise job fails), timestamps                                                                                                                                                                                                                                                           |
...
| Scan row         | `USER#<u>#SCAN#<scan_id>`     | `ROW#<scan_position, padded>`                   | scan_id, scan_position, filename, size_bytes, s3_key, status, suggestions (`external_source`, `external_id`, name, score), needs_review, error                                                                                                                                                                                                |
